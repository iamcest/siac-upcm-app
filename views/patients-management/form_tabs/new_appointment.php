<v-dialog v-model="patient_appointments.outside_dialog" max-width="90%"
    @click:outside="patient_appointments.outside_dialog = false">
    <v-card>
        <v-toolbar elevation="0">
            <v-toolbar-title>{{ AppointmentFormTitle }} - <span
                    v-if="editedItem.hasOwnProperty('first_name')">{{ editedItem.first_name + ' ' + editedItem.last_name }}</span>
            </v-toolbar-title>
            <v-spacer></v-spacer>
            <v-toolbar-items>
            ...
            <v-row class="px-8">
                <v-col cols="12" sm="6">
                    ...
                    <v-select class="mt-3" v-model="patient_appointments.editedItem.user_id"
                        ...
                        no-data-text='No hay datos disponibles' :loading="patient_appointments.select" outlined>
                    </v-select>
                </v-col>
                <v-col cols="12" sm="6">
                ...
                <v-col cols="12" sm="6">
                    ...
                    <v-select class="mt-3" v-model="patient_appointments.editedItem.appointment_type"
                        :items="patient_appointments.types" outlined></v-select>
                </v-col>
                <v-col cols="12" sm="6">
                    <label>Fecha de la cita</label>

                    <v-dialog ref="appointment_date_dialog" v-model="patient_appointments.date_modal"
                        :return-value.sync="patient_appointments.editedItem.appointment_date" width="290px">
                        <template #activator="{ on, attrs }">
                            <v-text-field class="mt-3" v-model="patient_appointments.editedItem.appointment_date"
                        ...
                        <v-date-picker v-model="patient_appointments.editedItem.appointment_date" locale="es"
                            scrollable>
                            <v-spacer></v-spacer>
                ...
                <v-col cols="12" sm="6">
                    ...
                    <v-dialog ref="appointment_time_dialog" v-model="patient_appointments.time_modal"
                        :return-value.sync="patient_appointments.editedItem.appointment_time" persistent width="290px">
                        <template #activator="{ on, attrs }">
                            <v-text-field class="mt-3" v-model="patient_appointments.editedItem.appointment_time"
                                append-icon="mdi-clock-time-four-outline" readonly v-bind="attrs" v-on="on" outlined>
                            </v-text-field>
                        </template>
