...
class Appointments
{
	...
	public function get_upcm_all($id = 0) {
	...
	public function create($data = []) {
		...
		extract($data);
		$sql = "INSERT INTO {$this->table} (appointment_reason, appointment_type, appointment_date, appointment_time, user_id, patient_id) VALUES('$appointment_reason', '$appointment_type', '$appointment_date', '$appointment_time', $user_id, $patient_id);";
	...
	public function finish ($id = 0) {
		...
		$sql = "UPDATE {$this->table} SET appointment_finished = 1 WHERE {$this->id_column} = $id;";
		$result = execute_query($sql);
		return $result;
	}
