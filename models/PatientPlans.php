...
class PatientPlans
{
    ...
    if (empty($result->fetch_object())) {
      $sql = "INSERT INTO {$this->table} (appointment_id, nutrition, exercise_plan, clinics_exams, patient_id) VALUES($appointment_id, '$nutrition', '$exercise_plan', '$clinics_exams', $patient_id);";
    }
    else {
      $sql = "UPDATE {$this->table} SET nutrition = '$nutrition', exercise_plan = '$exercise_plan', clinics_exams = '$clinics_exams' WHERE {$this->id_column} = $appointment_id";
    }
		$result = execute_query($sql);
		return $result;
	}
